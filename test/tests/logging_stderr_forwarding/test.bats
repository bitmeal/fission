...
@test "services: stderr forwarding [ON] - empty stdout" {
    ...
    run --separate-stderr -- docker run --rm -v ${CTX}/on.json:/etc/fission/fission.json -v ${CTX}/printer.js:/testbin/printer.js ${INIT} ${ARCH} ${IMAGE} sleep 2
    assert_success
...
@test "services: stderr forwarding [ON] - msg on stderr" {
    ...
    run -- docker run --rm -v ${CTX}/on.json:/etc/fission/fission.json -v ${CTX}/printer.js:/testbin/printer.js ${INIT} ${ARCH} ${IMAGE} sleep 2
    assert_success
...
@test "services: stderr forwarding [OFF]" {
    ...
    run -- docker run --rm -v ${CTX}/off.json:/etc/fission/fission.json -v ${CTX}/printer.js:/testbin/printer.js ${INIT} ${ARCH} ${IMAGE} sleep 2
    assert_success
