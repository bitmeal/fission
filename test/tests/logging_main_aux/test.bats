...
@test "aux process: main process redirected to logfile (no output on stdout)" {
    run -- docker run --rm -v ${CTX}/fission.json:/etc/fission/fission.json ${INIT} ${ARCH} ${IMAGE} echo fission-init -- sleep 1
    assert_success
...
@test "aux process: main process redirected to logfile (output in logfile)" {
    run --separate-stderr -- docker run --rm -v ${CTX}/fission.json:/etc/fission/fission.json ${INIT} ${ARCH} ${IMAGE} echo fission-init -- sh -c "sleep 1; cat /var/log/app/current"
    assert_success
