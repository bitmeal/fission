...
@test "config overlays: add to config" {
    run -- docker run --rm -v ${CTX}/add.json:/etc/fission/fission.json -v ${CTX}/overlays_add/:/etc/fission/overlays/ ${INIT} ${IMAGE} echo '${FOO}-${BAR}'
    assert_success
...
@test "config overlays: remove from config (key: null)" {
    run -- docker run --rm -v ${CTX}/rm.json:/etc/fission/fission.json -v ${CTX}/overlays_rm/:/etc/fission/overlays/ ${INIT} ${IMAGE} echo '${FOO}-${BAR}'
    assert_success
