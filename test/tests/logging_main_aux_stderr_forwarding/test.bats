...
@test "aux process: main stderr forwarding [ON]" {
    run --separate-stderr -- docker run --rm -v ${CTX}/on.json:/etc/fission/fission.json -v ${CTX}/echo_stderr.sh:/testbin/echo_stderr.sh ${INIT} ${ARCH} ${IMAGE} /testbin/echo_stderr.sh -- sleep 1
    assert_success
...
@test "aux process: main stderr forwarding [OFF]" {
    run -- docker run --rm -v ${CTX}/off.json:/etc/fission/fission.json  -v ${CTX}/echo_stderr.sh:/testbin/echo_stderr.sh ${INIT} ${ARCH} ${IMAGE} /testbin/echo_stderr.sh -- sleep 1
    assert_success
